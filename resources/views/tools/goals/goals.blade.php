            ...
            @if (isset($goals) && $goals->count() > 0)
                @foreach ($goals as $goal)
                    ...
                    <div
                        ...
                        <div class="flex flex-col sm:flex-row justify-between sm:items-end mb-2 relative z-10 gap-2 sm:gap-0">
                            <div>
                        ...
                        @if ($percent < 100)
                            <form method="POST" action="{{ route('finance.goals.add', $goal->id) }}"
                                ...
                                <input type="number" name="amount" placeholder="Сумма пополнения" required
                                    class="flex-1 bg-slate-950 border border-slate-800 rounded-lg px-4 py-2 text-sm text-slate-200 placeholder-slate-600 focus:border-emerald-500/50 focus:ring-0 font-medium"
                                    style="-moz-appearance: textfield;">
                                <button type="submit"
                                    class="w-full sm:w-auto px-5 py-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/20 font-bold text-sm transition-all shadow-sm">
                                    + Пополнить
                                </button>
                            </form>
                        @else
